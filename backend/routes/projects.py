...
async def create_project(project_data: ProjectItem, request: Request):
    ...
    existing_project = request.app.database.projects.find_one(
    ...
    return JSONResponse(
        {"id": str(project_query["_id"]), "title": project_query["title"]},
        status_code=201,
    )
...
def collect_stats(username: str, request: Request):
    ...
    if achievement is None:
        ...
        request.app.database.achievements.update_one(
            {"_id": achievement["_id"]}, {"$set": {"number": achievement["number"] + 1}}
        )
...
async def delete_project(project_id: str, request: Request):
    ...
    if project["admin"] != user_query["username"]:
        return {
            "message": "You have no rights to change status of project. You have to be admin for that!"
        }

    request.app.database.projects.update_one(
        {"_id": ObjectId(project_id)}, {"$set": {"status": "CANCELLED"}}
    )
    project = request.app.database.projects.find_one({"_id": ObjectId(project_id)})
...
async def show_projects(request: Request, page: int = 0, perPage: int = 12):
    ...
    resp = {"data": result, "metadata": {"total": count}}
...
async def change_status(project_id: str, status: str, request: Request):
    ...
    request.app.database.projects.update_one(
        {"_id": ObjectId(project_id)}, {"$set": {"status": status}}
    )
    project = request.app.database.projects.find_one({"_id": ObjectId(project_id)})
    return JSONResponse({"id": str(project["_id"])}, status_code=200)
...
#         project["admin"], decoded_jwt["username"], project["_id"], request
#     )

#     resp = {"user": decoded_jwt["username"], "project": project["title"]}
#     return JSONResponse(resp, status_code=200)


# @router.post("/${project_id}/invite_to_project")
# async def add_member(
#     user2invite: UserItem, project_data: ProjectItem, request: Request
# ):
#     authorization = jwt_auth.get_authorisation(request)
#     decoded_jwt = jwt_auth.decode_jwt(authorization)
...
#         return {
#             "message": "You have no rights to invite users to project. You have to be admin for that!"
#         }

#     proj_and_user_query = {
#         "username": newuser_query["username"],
#         "title": project_query["title"],
#         "approved": False,
#     }
#     request.app.database.user2project.insert_one(proj_and_user_query)
...
#     await send_verification_email(
#         newuser_query["username"],
#         newuser_query["username"],
#         project_query["_id"],
#         request,
#     )

#     resp = {"user": decoded_jwt["username"], "project": project_query["title"]}
#     return JSONResponse(resp, status_code=200)
