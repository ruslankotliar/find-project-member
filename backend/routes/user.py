...
async def login_user(login_data: AuthItem, request: Request):
    ...
    if user:
        if jwt_auth.verify_password(user["password"], user_query["password"]):
            ...
            return JSONResponse(
                {"token": encoded_jwt, "id": str(user["_id"])}, status_code=200
            )
        else:
            return JSONResponse(
                {"message": "Login failed. Wrong password."}, status_code=400
            )
    else:
        return JSONResponse(
            {"message": "Login failed. User not found."}, status_code=404
        )
...
async def add_userinfo(user_data: UserItem, request: Request):
    ...
    user = request.app.database.users.find_one(user_query)
    return JSONResponse(
        {"token": authorization, "id": str(user["_id"])}, status_code=200
    )
...
async def user_projects(request: Request, page: int = 0, perPage: int = 12):
    ...
    if authorization:
        ...
        resp = {"data": projects, "metadata": {"total": total}}
